package org.yinan.core.view.window.frame;
...
import org.yinan.core.service.RestTopic;
import org.yinan.core.utils.RestUtil;
import org.yinan.core.utils.SystemUtil;
import org.yinan.core.view.window.RestfulTreeCellRenderer;
...
public class RightToolWindow extends JPanel {
    ...
    private void initEvent() {
        ...
        project.getMessageBus().connect().subscribe(RestTopic.ACTION_SCAN_SERVICE, data -> {
            if (data instanceof Map) {
                ...
                renderRequestTree((Map<String, Map<String, List<Request>>>) data);
            }
        });
        ...
        tree.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (SwingUtilities.isLeftMouseButton(e)) {
                    ...
                    Request node;
                    if (e.getClickCount() == doubleClick && (node = getTreeNodeRequest(tree)) != null) {
                        node.navigate(true);
                    }
                }
            }
     ...
     * 渲染Restful请求列表
     */
    public void renderRequestTree(@NotNull Map<String, Map<String, List<Request>>> allRequest) {
        ...
        allRequest.forEach((moduleName, requestMaps) -> {
            //统计模块下面有多少个controller
            DefaultMutableTreeNode moduleItem = new DefaultMutableTreeNode(String.format(
                    "[%d]%s",
                    requestMaps.size(),
                    moduleName
            ));

            requestMaps.forEach((controllerName, requests) -> {
                DefaultMutableTreeNode controllerItem = new DefaultMutableTreeNode(String.format(
                        "[%d]%s",
                        requests.size(),
                        controllerName
                ));
                requests.forEach(request -> {
                    controllerItem.add(new DefaultMutableTreeNode(request));
                    controllerCount.incrementAndGet();
                });
                moduleItem.add(controllerItem);
            });
