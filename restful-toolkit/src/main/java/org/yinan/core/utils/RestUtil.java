...
import org.yinan.core.beans.PropertiesKey;
import org.yinan.core.beans.Request;
import org.yinan.core.utils.scanner.JaxrsHelper;
import org.yinan.core.utils.scanner.SpringHelper;
...
public class RestUtil {
     ...
     */
    @NotNull
    public static Map<String, Map<String, List<Request>>> getAllRequest(@NotNull Project project) {
        return getAllRequest(project, false);
    }

    public static List<Request> allRequests(@NotNull Project project) {
        Map<String, Map<String, List<Request>>> mapRequests = getAllRequest(project);
        return mapRequests.values()
                .stream()
                .flatMap(re -> re
                        .values()
                        .stream()
     ...
     */
    @NotNull
    public static Map<String, Map<String, List<Request>>> getAllRequest(@NotNull Project project, boolean hasEmpty) {
        Map<String, Map<String, List<Request>>> map = new HashMap<>();
        //获取所有模块
        Module[] modules = ModuleManager.getInstance(project).getModules();
        for (Module module : modules) {
            //获取当前模块的所有请求
            Map<String, List<Request>> requests = getAllRequestByModule(project, module);
     ...
     */
    @NotNull
    public static Map<String, List<Request>> getAllRequestByModule(@NotNull Project project, @NotNull Module module) {
        // JAX-RS方式，忽略
...
//        }
        ...
        // Spring RESTFul方式
        Map<String, List<Request>> springRequestByModule = SpringHelper.getSpringRequestByModule(project, module);
    ...
    public static List<Request> getAllRequestByModule(@NotNull Module module, @NotNull Project project) {
        Map<String, List<Request>> requestsByModule = getAllRequestByModule(project, module);
        return requestsByModule.values().stream().flatMap(Collection::stream).collect(Collectors.toList());
    }
    ...
    /**
     * 获取属性值，将传过来的属性值进行解析，如果是常量，返回常量值，如果是枚举，返回枚举字段名称，如果是数组，转换成list
     *
